...
float stopTimerHesaff(const struct timeval &start, const std::string &label){
	float time = timeElapsedHesaff(start);
	if(!label.empty())
...
void photometricallyNormalize(float *image, const float *binaryMask, float &sum, float &var, const int width, const int height){
	   ...
	   for (int j=0; j < height; j++)
	      for (int i=0; i < width; i++)
	         if (binaryMask[j*width+i]>0)
	            var += (sum - image[j*width+i])*(sum - image[j*width+i]);
	   ...
	   for (int j=0; j < height; j++)
	      for (int i=0; i < width; i++)
	      {
	         image[j*width+i] = 128 + fac * (image[j*width+i] - sum);
	         if (image[j*width+i] > 255) image[j*width+i]=255;
	         if (image[j*width+i] < 0)   image[j*width+i]=0;
	      }
}
